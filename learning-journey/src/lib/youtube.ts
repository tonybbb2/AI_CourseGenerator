...
export async function getQuestionsFromTranscript(transcript : string, course_title : string){
    ...
    const questions : Question[] = await strict_output(
        ...
        new Array(5).fill(
            `You are to generate a random card mcq question about ${course_title} with context of the following transcript : ${transcript}`
        ),
        {
            question : 'question',
            answer : 'answer with max length of 15 words',
            option1 : 'option1 with max length of 15 words',
            option2 : 'option1 with max length of 15 words',
            option3 : 'option1 with max length of 15 words',
        }
    );
    return questions;
}